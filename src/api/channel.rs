...
fn store_receive_data(dst: ObjectPtr, data: Option<ObjectPtr>, size: usize) {
    if let Some(src) = data {
        let src = unsafe { slice::from_raw_parts(src.0 as *const u8, size) };
        let dst = unsafe { slice::from_raw_parts_mut(dst.0 as *mut u8, size) };
...
pub extern "C" fn gox5_channel_select(ctx: &mut LightWeightThreadContext) -> FunctionObject {
    fn set_result(
        ...
        *frame.selected_index = match selected_index {
            Some(index) => isize::try_from(index).unwrap(),
            None => -1,
        };
        *frame.receive_available = receive_available;
    }
